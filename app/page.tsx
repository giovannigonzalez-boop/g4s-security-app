  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center', backgroundColor: '#F3F4F6', fontFamily: 'sans-serif' }}>
      <div style={{ backgroundColor: 'white', padding: '50px', borderRadius: '24px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', width: '380px', textAlign: 'center' }}>
        <div style={{ backgroundColor: '#E11D48', color: 'white', width: '70px', height: '70px', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '24px', margin: '0 auto 25px', boxShadow: '0 5px 15px rgba(225,29,72,0.3)' }}>G4S</div>
        <h2 style={{ fontSize: '22px', fontWeight: '800', marginBottom: '10px' }}>ARC SECURITY</h2>
        <p style={{ color: '#6B7280', fontSize: '14px', marginBottom: '30px' }}>Ingrese sus credenciales de operador</p>
        <input type="text" placeholder="Usuario" style={inputStyle} />
        <input type="password" placeholder="Contraseña" style={inputStyle} />
        <button onClick={onLogin} style={{ width: '100%', padding: '14px', backgroundColor: '#E11D48', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer', marginTop: '10px' }}>Ingresar al Sistema</button>
      </div>
    </div>
  );
}
...
function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  // BUSCADOR CORREGIDO: Filtra por nombre de cliente o cuenta
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#F8FAFC', fontFamily: 'sans-serif' }}>
      ...
      {/* SIDEBAR CON LOGO G4S ARC SECURITY */}
      <nav style={{ width: '280px', backgroundColor: 'white', borderRight: '1px solid #E2E8F0', padding: '30px 20px', position: 'fixed', height: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: '11px', color: '#94A3B8', fontWeight: '800', textTransform: 'uppercase', marginBottom: '15px' }}>Simulador Central</p>
        ...
        <button onClick={onLogout} style={{ border: 'none', background: 'none', borderTop: '1px solid #F1F5F9', padding: '20px 0', display: 'flex', alignItems: 'center', gap: '10px', color: '#E11D48', fontWeight: 'bold', cursor: 'pointer' }}>
          <LogOut size={22}/> Finalizar Sesión
        </button>
      </nav>

      <main style={{ flex: 1, marginLeft: '280px', padding: '40px' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
          <h1 style={{ fontSize: '26px', fontWeight: '800', color: '#0F172A' }}>Operaciones G4S</h1>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '30px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
            ...
            {filtered.map(l => (
              <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 0', borderBottom: '1px solid #F1F5F9' }}>
                <div style={{ display: 'flex', gap: '15px' }}>
                  <div style={{ padding: '10px', borderRadius: '12px', backgroundColor: l.tipo_evento?.includes('PÁNICO') ? '#FFF1F2' : '#F1F5F9' }}>
                    {l.tipo_evento?.includes('PÁNICO') ? <AlertTriangle size={20} color="#E11D48"/> : <ShieldCheck size={20} color="#64748B"/>}
                  </div>
                  <div>
                ...
                {l.tipo_evento?.includes('PÁNICO') && (
                  <button onClick={() => simulate('FALSA ALARMA (ANULADA)', l.nombre_cliente, l.cuenta)} style={{ padding: '6px 12px', backgroundColor: '#FFF1F2', color: '#E11D48', border: '1px solid #E11D48', borderRadius: '8px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}>Falsa Alarma</button>
                )}
              </div>
            ))}
          </div>
